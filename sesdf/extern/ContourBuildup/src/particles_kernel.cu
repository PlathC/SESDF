...
///////////////////////////////////////////////////////////////////////////////
__device__ uint countNeighborsInCell( uint *   neighbors,	  // output: neighbor indices
		...
		for ( uint j = startIndex; j < endIndex; j++ )
		{
			...
			if ( j != index )
			{
				...
				if ( dist < neighborDist )
				{
					...
					if ( ( neighborIndex + count ) >= params.maxNumNeighbors )
						return count;
...
// count all neighbor atoms in a given cell
///////////////////////////////////////////////////////////////////////////////
__device__ uint countProbeNeighborsInCell( // uint*   neighbors,     // output: neighbor indices
	...
	{ // cell is not empty
		...
		for ( uint j = startIndex; j < endIndex; j++ )
		{
			...
			if ( j != index )
			{
				...
				{
					// check number of neighbors
					if ( ( neighborIndex + count ) >= rsParams.maxNumProbeNeighbors )
						return count;
					neighbors[ atomIndex * rsParams.maxNumProbeNeighbors + neighborIndex + count ]
						= make_float3( pos2.x, pos2.y, pos2.z );
					// increment the neighbor counter
					count++;
				}
			}
...
// counting function
///////////////////////////////////////////////////////////////////////////////
__global__ void countNeighbors( uint *	 neighborCount,		// output: number of neighbors
	...
	for ( int z = ( start.z > 0 ? start.z : 0 ); z < ( end.z > gridSize.z ? gridSize.z : end.z ); z++ )
	{
		for ( int y = ( start.y > 0 ? start.y : 0 ); y < ( end.y > gridSize.y ? gridSize.y : end.y ); y++ )
		{
			for ( int x = ( start.x > 0 ? start.x : 0 ); x < ( end.x > gridSize.x ? gridSize.x : end.x ); x++ )
			{
				...
				count += countNeighborsInCell( neighbors,
											   smallCircles,
											   count,
											   originalIndex,
											   neighborPos,
...
// counting function
///////////////////////////////////////////////////////////////////////////////
__global__ void countProbeNeighbors( // uint*   probeNeighborCount, // output: number of neighbors
	...
	uint	 numProbes )
{
	...
	for ( int z = ( start.z > 0 ? start.z : 0 ); z < ( end.z > gridSize.z ? gridSize.z : end.z ); z++ )
	{
		for ( int y = ( start.y > 0 ? start.y : 0 ); y < ( end.y > gridSize.y ? gridSize.y : end.y ); y++ )
		{
			...
			{
				neighborPos = make_int3( x, y, z );
				count += countProbeNeighborsInCell( probeNeighbors,
													count,
													originalIndex,
													neighborPos,
													index,
...
// compute the arcs
///////////////////////////////////////////////////////////////////////////////
__global__ void computeArcs( float4 * arcs,				 // output: arcs
							 ...
							 uint	  numAtoms )
{
	...
	for( uint cnt = 0; cnt < numNeighbors; ++cnt ) {
		...
		d = sk * dot( cross( make_float3( arcs[origAtomIdx * params.maxNumNeighbors * 4 + 0]), p1), make_float3(
	arcs[origAtomIdx * params.maxNumNeighbors * 4 + 1]));
...
///////////////////////////////////////////////////////////////////////////////
__global__ void computeReducedSurface( uint4 *	point1,			   // output: the atom indices
									   ...
									   uint *	visibleAtomsId )
{ // input: visible atoms original index list
	...
	tij = 0.5f * ( ai + aj )
		  + 0.5f * ( aj - ai )
				* ( ( ri + params.probeRadius ) * ( ri + params.probeRadius )
					- ( rj + params.probeRadius ) * ( rj + params.probeRadius ) )
				/ ( dij * dij );
	tik = 0.5f * ( ai + ak )
		  + 0.5f * ( ak - ai )
				* ( ( ri + params.probeRadius ) * ( ri + params.probeRadius )
					- ( rk + params.probeRadius ) * ( rk + params.probeRadius ) )
				/ ( dik * dik );
	...
	if ( draw0 && draw1 )
	{
		point1[ visibleAtomIdx * params.maxNumNeighbors * params.maxNumNeighbors + idxX ]
			= make_uint4( aiIdx, ajIdx, visibleAtomIdx, 1 );
		probePos[ visibleAtomIdx * params.maxNumNeighbors * params.maxNumNeighbors + idxX ]
			= make_float4( pijk0.x, pijk0.y, pijk0.z, -1.0f );
	}
	else if ( draw0 && !draw1 )
	{
		point1[ visibleAtomIdx * params.maxNumNeighbors * params.maxNumNeighbors + idxX ]
	...
	else if ( !draw0 && draw1 )
	{
		point1[ visibleAtomIdx * params.maxNumNeighbors * params.maxNumNeighbors + idxX ]
			= make_uint4( aiIdx, ajIdx, visibleAtomIdx, 1 );
		probePos[ visibleAtomIdx * params.maxNumNeighbors * params.maxNumNeighbors + idxX ]
			= make_float4( pijk1.x, pijk1.y, pijk1.z, 1.0f );
	}
	else
	{
		return;
...
///////////////////////////////////////////////////////////////////////////////
__global__ void computeTriangleVBO( float3 * vbo,	  // output: triangle vertices and colors
...
// compute the triangle vbo
///////////////////////////////////////////////////////////////////////////////
__global__ void computeVisibleTriangleVBO( float3 * vbo,		  // output: triangle vertices and colors
										   uint4 *	point1,		  // input: point 1 of the RS face
										   float4 * atomPos,	  // input: sorted atom positions
										   float4 * visibleAtoms, // input: visible atoms position and radius
										   uint		offset )
...
///////////////////////////////////////////////////////////////////////////////
__global__ void computeTorusVBO(
...
{ // the probe position array
	...
	rij = 0.5f
		  * sqrt(
			  float( ( ri + rj + 2.0f * params.probeRadius ) * ( ri + rj + 2.0f * params.probeRadius ) - dij * dij ) )
		  * ( sqrt( float( dij * dij - ( ri - rj ) * ( ri - rj ) ) ) / dij );
	rik = 0.5f
		  * sqrt(
			  float( ( ri + rk + 2.0f * params.probeRadius ) * ( ri + rk + 2.0f * params.probeRadius ) - dik * dik ) )
		  * ( sqrt( float( dik * dik - ( ri - rk ) * ( ri - rk ) ) ) / dik );
	rjk = 0.5f
		  * sqrt(
			  float( ( rj + rk + 2.0f * params.probeRadius ) * ( rj + rk + 2.0f * params.probeRadius ) - djk * djk ) )
		  * ( sqrt( float( djk * djk - ( rj - rk ) * ( rj - rk ) ) ) / djk );
...
///////////////////////////////////////////////////////////////////////////////
__global__ void writeProbePositions( float4 * probePos, // output (probe positions)
...
///////////////////////////////////////////////////////////////////////////////
__global__ void writeSingularities( float3 * outArray, uint * probeNeighbors, float4 * probePos )
{
	// get the indices
...
///////////////////////////////////////////////////////////////////////////////
__global__ void findAdjacentTriangles( float *	outPbo,
									   uint4 *	point1,
									   float4 * probePos,
									   uint *	neighborCount,
									   uint *	neighbors,
									   ...
									   uint *	visibleAtomsId )
{
	...
	for ( cnt = 0; cnt < counter; ++cnt )
	{
		...
		tmpDir = normalize(
			make_float3( invisibleTriaNormal[ cnt ].x, invisibleTriaNormal[ cnt ].y, invisibleTriaNormal[ cnt ].z )
			- tik );
...
__device__ uint findNeighborsInCellCBCuda( uint *	neighbors,	   // out: neighbor indices
		...
		for ( uint j = startIndex; j < endIndex; j++ )
		{
			...
			if ( j != index )
			{
				...
				if ( dist < neighborDist )
				{
					...
					// compute small circle / intersection plane
					r = ( ( pos.w + params.probeRadius ) * ( pos.w + params.probeRadius ) ) + ( dist * dist )
						- ( ( pos2.w + params.probeRadius ) * ( pos2.w + params.probeRadius ) );
					r	= r / ( 2.0f * dist * dist );
					vec = relPos * r;
					// set small circle
					...
					// smallCircle.w = 1.0f;
					smallCircle.w
						= sqrt( ( ( pos.w + params.probeRadius ) * ( pos.w + params.probeRadius ) ) - dot( vec, vec ) );
...
__global__ void findNeighborsCBCuda( uint *	  neighborCount, // out: number of neighbors
									 ...
									 uint	  numAtoms )
{
	...
	for ( int z = ( start.z > 0 ? start.z : 0 ); z < ( end.z > gridSize.z ? gridSize.z : end.z ); z++ )
	{
		for ( int y = ( start.y > 0 ? start.y : 0 ); y < ( end.y > gridSize.y ? gridSize.y : end.y ); y++ )
		{
			...
			{
				neighborPos = make_int3( x, y, z );
				count += findNeighborsInCellCBCuda(
					neighbors, smallCircles, count, neighborPos, index, pos, atomPos, cellStart, cellEnd );
			}
		}
	}
...
__global__ void removeCoveredSmallCirclesCBCuda( float4 * smallCircles,		  // in/out: small circles
												 ...
												 uint	  numAtoms )
{
	...
	if ( atomIdx >= numAtoms )
		return;
	// check, if neighbor index is within bounds
	if ( jIdx >= params.maxNumNeighbors )
		return;
	...
	for ( uint kCnt = 0; kCnt < numNeighbors; kCnt++ )
	{
		...
		if ( jIdx != kCnt )
		{
			...
			h = vj * ( dot( vj, vj - vk ) * dot( vk, vk ) ) / denom
				+ vk * ( dot( vk - vj, vk ) * dot( vj, vj ) ) / denom;
			// compute cases
			nj = normalize( pi - pj );
			nk = normalize( pi - pk );
			q  = vk - vj;
...
__global__ void computeArcsCBCuda( uint *	smallCircleVisible, // out: small circle visibility
								   ...
								   uint		numAtoms )
{
	...
	for ( aCnt = 0; aCnt < arcCnt; aCnt++ )
	{
		if ( atomIdx < j )
		{
			...
			{
				arcs[ atomIdx * params.maxNumNeighbors * params.maxNumNeighbors + jIdx * params.maxNumNeighbors
					  + arcWritten ]
					= make_float4( pi + vj + ( cos( start[ aCnt ] ) * xAxis + sin( start[ aCnt ] ) * yAxis ) * scj.w,
								   float( startkIndex[ aCnt ] ) + 0.2f ); // start[aCnt]);
				arcWritten++;
			}
			if ( j < endkIndex[ aCnt ] )
			{
				arcs[ atomIdx * params.maxNumNeighbors * params.maxNumNeighbors + jIdx * params.maxNumNeighbors
					  + arcWritten ]
					= make_float4( pi + vj + ( cos( end[ aCnt ] ) * xAxis + sin( end[ aCnt ] ) * yAxis ) * scj.w,
								   float( endkIndex[ aCnt ] ) + 0.2f ); // end[aCnt]);
				arcWritten++;
			}
		}
	}
...
__global__ void writeProbePositionsCBCuda( float4 * probePos,		// out: the probe positions
										   ...
										   uint		numAtoms )
{
	...
	{
		tmpProbePos
			= arcs[ atomIdx * params.maxNumNeighbors * params.maxNumNeighbors + jIdx * params.maxNumNeighbors + cnt ];
		probePos[ numPreviousProbes + cnt ]
			= make_float4( tmpProbePos.x, tmpProbePos.y, tmpProbePos.z, params.probeRadius );
		...
		sphereTriaVec3[ numPreviousProbes + cnt ]
			= make_float4( make_float3( ak ) - make_float3( tmpProbePos ), params.probeRadius * params.probeRadius );
	}
...
__device__ uint findProbeNeighborsInCellCBCuda( float3 * singTex,		 // out: singularity texture
												uint	 neighborIndex,	 // in: first index for writing in neighbor list
												int3	 gridPos,		 // in: the current grid cell
	...
	{ // cell is not empty
		...
		for ( uint j = startIndex; j < endIndex; j++ )
		{
			...
			if ( j != index )
			{
				...
				{
					// check number of neighbors
					if ( ( neighborIndex + count ) >= numNeighbors )
						return count;
					// write singTex
...
__global__ void findProbeNeighborsCBCuda( float3 * texCoord,	   // out: #neighbors + texture coords
										  ...
										  uint	   numNeighbors )
{
	...
	for ( int z = ( start.z > 0 ? start.z : 0 ); z < ( end.z > gridSize.z ? gridSize.z : end.z ); z++ )
	{
		for ( int y = ( start.y > 0 ? start.y : 0 ); y < ( end.y > gridSize.y ? gridSize.y : end.y ); y++ )
		{
			...
			{
				neighborPos = make_int3( x, y, z );
				count += findProbeNeighborsInCellCBCuda( singTex,
														 count,
														 neighborPos,
														 index,
														 pos,
	...
	texCoord[ gridProbeIndex[ index ] ]
		= make_float3( float( count ), float( xCoord ), float( index % params.texSize ) );
}
