...
extern "C"
{
	...
	void threadSync() { checkCudaErrors( cudaThreadSynchronize() ); }
	...
	void registerGLBufferObject( uint vbo ) { checkCudaErrors( cudaGLRegisterBufferObject( vbo ) ); }

	void unregisterGLBufferObject( uint vbo ) { checkCudaErrors( cudaGLUnregisterBufferObject( vbo ) ); }
	...
	void unmapGLBufferObject( uint vbo ) { checkCudaErrors( cudaGLUnmapBufferObject( vbo ) ); }
						 ...
						 uint	 numCells )
	{
		...
		checkCudaErrors(
			cudaBindTexture( 0, smallCirclesTex, smallCircles, numAtoms * numNeighbors * sizeof( float4 ) ) );
						  ...
						  uint	  numCells )
	{
		...
		// execute the kernel
		countNeighbors<<<numBlocks, numThreads>>>(
			neighborCount, neighbors, (float4 *)sortedPos, gridParticleIndex, cellStart, cellEnd, numAtoms );
									...
									uint	numNeighbors )
	{
		...
		numBlocks.y = numVisibleAtoms / numThreads.y + ( numVisibleAtoms % numThreads.y == 0 ? 0 : 1 );
		numBlocks.z = 1;
		// execute the kernel
	...
	void computeTriangleVBOCuda( float3 * vbo,
								 uint4 *  point1,
								 // float* point2, float* point3,
								 float * atomPos,
								 float * visibleAtoms,
	...
	void computeVisibleTriangleVBOCuda( float3 *	vbo,
										uint4 *		point1,
	...
	{ // the number of visible triangles
		...
		uint numThreads, numBlocks;
		numThreads = 8;
		numBlocks  = numVisibleTria / numThreads + ( numVisibleTria % numThreads == 0 ? 0 : 1 );
		// execute the kernel
		computeTorusVBO<<<numBlocks, numThreads>>>( outTorusVBO,
													outSTriaVBO,
													inVBO,
													(float4 *)atomPos,
													(float4 *)visibleAtoms,
	...
	void writeProbePositionsCuda( float *  probePos, // output (probe positions)
	...
	{ // the number of probes
		...
		uint numThreads, numBlocks;
		numThreads = 8;
		numBlocks  = numProbes / numThreads + ( numProbes % numThreads == 0 ? 0 : 1 );
		// execute the kernel
	...
	void writeSingularitiesCuda( float3 * outArray,
								 uint *	  probeNeighbors,
								 float *  probePos,
								 uint	  numProbes,
								 uint	  numNeighbors )
	{
		...
		numBlocks.y = numProbes / numThreads.y + ( ( numProbes % numThreads.y ) == 0 ? 0 : 1 );
		numBlocks.z = 1;
		// execute the kernel
		writeSingularities<<<numBlocks, numThreads>>>( outArray, probeNeighbors, (float4 *)probePos );

		// check if kernel invocation generated an error
		getLastCudaError( "Kernel execution (writeSingularities) failed" );
	}

	void findAdjacentTrianglesCuda( float *		outPbo,
									cudaArray * visibility,
									uint *		point1,
									float *		probePos,
									uint *		neighborCount,
									...
									uint		numNeighbors )
	{
		...
		numBlocks.z = 1;
		// execute the kernel
		findAdjacentTriangles<<<numBlocks, numThreads>>>( outPbo,
														  (uint4 *)point1,
														  (float4 *)probePos,
														  neighborCount,
														  neighbors,
	...
	void findNeighborsCB( uint *  neighborCount,
						  uint *  neighbors,
						  float * smallCircles,
						  float * sortedPos,
						  uint *  cellStart,
						  ...
						  uint	  numCells )
	{
		...
		// execute the kernel
		findNeighborsCBCuda<<<numBlocks, numThreads>>>(
			neighborCount, neighbors, (float4 *)smallCircles, (float4 *)sortedPos, cellStart, cellEnd, numAtoms );
	...
	void removeCoveredSmallCirclesCB( float * smallCircles,
	...
	void computeArcsCB( float * smallCircles,
						uint *	smallCircleVisible,
						uint *	neighborCount,
						uint *	neighbors,
						float * sortedPos,
						...
						uint	numNeighbors )
	{
		...
		// execute the kernel
		computeArcsCBCuda<<<numBlocks, numThreads>>>( smallCircleVisible, (float4 *)arcs, arcCount, numAtoms );
	...
	void writeProbePositionsCB( float * probePos,
								...
								uint	numNeighbors )
	{
		...
		writeProbePositionsCBCuda<<<numBlocks, numThreads>>>( (float4 *)probePos,
															  (float4 *)sphereTriaVec1,
															  (float4 *)sphereTriaVec2,
															  (float4 *)sphereTriaVec3,
															  (float4 *)torusPos,
															  (float4 *)torusVS,
															  (float4 *)torusAxis,
															  (float4 *)arcs,
	...
	void writeSingularityTextureCB( float * texCoord,
									float * singTex,
									float * sortedProbePos,
									uint *	gridProbeIndex,
									uint *	cellStart,
									...
									uint	numCells )
	{
		...
		//// execute the kernel
		findProbeNeighborsCBCuda<<<numBlocks, numThreads>>>( (float3 *)texCoord,
															 (float3 *)singTex,
															 (float4 *)sortedProbePos,
															 gridProbeIndex,
															 cellStart,
